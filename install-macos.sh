...
offer_claude_fallback() {
  ...
  local prompt="CITADEL installer failed at step \"${step}\". Error: ${err_msg}. Script: install.sh${line_info}. Repo: https://github.com/romcsibacsi/citadel. OS: macOS $(sw_vers -productVersion 2>/dev/null || echo unknown). Node: $(node -v 2>/dev/null || echo missing). Dir: ${INSTALL_DIR}. Your task: diagnose this CITADEL installer failure. The install scripts are install.sh (macOS) and install-linux.sh. Read the relevant section, check for missing dependencies or permission issues, and suggest concrete shell commands to fix."
  if [ -t 0 ]; then
...
echo -e "${BOLD} ▝▜█████▛▘  A sajat AI csapatod, ami non-stop fut.${NC}"
echo -e "${DIM}   ▘▘ ▝▝${NC}"
echo ""
...
if [ "$CHANNEL_PROVIDER" = "telegram" ]; then
  ...
  SLACK_ENTRY='{"plugin":"slack-channel","marketplace":"citadel-marketplace"}'
...
try:
  ...
  sys.exit(0 if any(p.get('plugin')=='slack-channel' and p.get('marketplace')=='citadel-marketplace' for p in plugins) else 1)
except: sys.exit(1)
...
# fallback to "nexus" so we never end up with a blank identifier.
...
# slug rather than hardcoded "nexus". Skip task dirs that already exist --
...
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "${BOLD}${GREEN}  ✓ CITADEL sikeresen telepítve!${NC}"
echo ""
