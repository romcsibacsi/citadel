...
echo ""
echo -e "${BOLD}CITADEL frissites...${NC} [$(date -u +%Y-%m-%dT%H:%M:%SZ)]"
echo ""
...
if [ -n "$DIRTY" ]; then
  if [ "${AUTO_STASH:-0}" = "1" ]; then
    ...
    if ! git stash push --keep-index -m "citadel-update-auto-stash $(date +%Y%m%d-%H%M%S)"; then
...
SLACK_REF_FILE="$INSTALL_DIR/store/citadel-marketplace-slack-channel-ref.txt"
MARKETPLACE_PLUGIN_DIR="$HOME/.claude/plugins/cache/claude-plugins-official/slack-channel"
