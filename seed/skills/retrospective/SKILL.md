...
agentctl mem search "today"

# Today's daily log entries are visible a dashboardról

# Skills that were referenced or used (this agent's own skills)
ls skills/ | head -30
```
...
- Check the shared skills under `seed/skills/` and this agent's own `skills/` dir before proposing duplicates
...
On user approval:
- Skills: create/patch SKILL.md files (shared in `seed/skills/`, agent-own in `skills/`)
- Memory: write/update via `agentctl mem save <hot|warm|cold|shared> <txt>`
- Workflow: update CLAUDE.md or message affected agents via `agentctl msg send <agent> <txt>`

After execution, log the retrospective to the daily log:
```bash
agentctl log "Retrospective -- [count] skill changes, [count] memory updates, [count] workflow changes applied."
```
