...
# Hot memories (active task state)
agentctl mem search hot

# Recent warm memories (project context)
agentctl mem search warm

# Today's daily log — review your recent log lines from the dashboard
```
...
```bash
agentctl msg send TARGET "[HANDOFF] purpose: ...
...
- If `target=` agent is not running (check the dashboard/activity board), warn and fall back to file mode
...
|-----------|-------|--------------------|
| checkpoint | Session summary | Source: pulls recent checkpoint data |
