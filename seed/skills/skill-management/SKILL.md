...
description: List, inspect, patch, delete, or extract skills. Global/shared skills live under seed/skills/; an agent's own skills live in that agent's skills/ directory. Use when the user asks about available skills, wants to modify an existing skill, when a retrospective proposes skill changes, or to turn a demonstrated workflow/conversation into a reusable SKILL.md. Trigger on "/skills" command, skill-related retrospective actions, "csinálj ebből skill-t", "jegyezd meg ezt a folyamatot", or after a complex repeatable task.
---
...
|----------|----------|-------------|
| `action` | YES | One of: `list`, `show`, `create`, `extract`, `patch`, `delete`, `audit` |
| `name` | for show/patch/delete | Skill directory name (e.g. `github-pr-rebase-merge`) |
| `scope` | no | `global` (default, the shared `seed/skills/` library) or `local` (the agent's own `skills/` directory) |
...
- `/skills action=extract name=new-skill` -- distill a demonstrated workflow into a SKILL.md
...
```bash
# Global/shared skills live under seed/skills/; an agent's own skills live in its skills/ dir.
SKILL_DIR="seed/skills"
if [ "$SCOPE" = "local" ]; then SKILL_DIR="skills"; fi
...
1. Read `seed/skills/{name}/SKILL.md` (or the agent's `skills/{name}/SKILL.md` for local scope)
...
4. Write to `seed/skills/{name}/SKILL.md` (global) or the agent's `skills/{name}/SKILL.md` (local)
...
### action=extract (skill-factory)

Turn a demonstrated workflow or conversation into a reusable SKILL.md.
Triggers: "csinálj ebből skill-t", "jegyezd meg ezt a folyamatot", or after a
complex repeatable task has just been completed successfully.

## Eljárás

1. Scope-check: only extract if the task is repeatable (the 2+ occurrence rule);
   refuse one-off work.
2. Read back the demonstrated workflow from the transcript/conversation and
   identify the durable parts: the trigger condition, the ordered steps actually
   taken, and the decision points.
3. Generalize: strip one-off specifics (concrete IDs, paths, sample data) into
   named arguments; keep the procedure, not the example.
4. Capture pitfalls: fold any errors hit and their recoveries into a Pitfalls
   section so the next run avoids them.
5. Write the SKILL.md using the `action=create` template, then hand off to
   `action=create`'s rules (under 500 lines, specific description, `references/`
   for bulk material) and regenerate `.skill-index.md`.
6. Redaction: never bake secrets, tokens, or live values into the extracted skill.
...
3. On confirmation:
```bash
# global: seed/skills/${NAME}  |  local: skills/${NAME}
rm -rf "seed/skills/${NAME}"
```
...
```bash
if [ -f seed/skills/.skill-index.md ]; then
  echo "Index exists, checking sync..."
  # Compare index entries vs actual directories
  indexed=$(grep -oP '(?<=\[)[^\]]+' seed/skills/.skill-index.md | sort)
  actual=$(ls -d seed/skills/*/ 2>/dev/null | xargs -I{} basename {} | sort)
...
```bash
INDEX_FILE="seed/skills/.skill-index.md"
