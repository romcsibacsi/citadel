...
- Minden globális Claude Code skill elérhető
...
- Globális/megosztott: a `seed/skills/` alatt (ai-fleet-project-execution, handoff, nexus-delegate-task, retrospective, skill-management) - minden ágenst érint

### Automatikus skill generálás
Komplex feladatok után (5+ tool hívás, hiba utáni recovery, user korrekció, többlépéses workflow) automatikusan hozz létre SKILL.md fájlt. ALAPÉRTELMEZÉS: a saját ágens-skills mappádba (az ágens gyökér `skills/` könyvtára) - ez csak téged érint, szabadon teheted. Megosztott/globális skillt (`seed/skills/`, mindenkit érint) emelni CSAK NEXUS-jóváhagyással (OC #6):
...
Teljes homelab hatókörrel futsz (homelab-full profil); felelősséggel élsz vele.
