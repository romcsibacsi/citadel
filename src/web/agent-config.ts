...
// Per-agent UI accent (hex). Phase 7B: surfaced to the dashboard so non-base
// agents can drive their framed-avatar ring (--ac) from real config too.
// Returns null when unset/invalid; the frontend then falls back to a neutral.
export function readAgentAccent(name: string): string | null {
  const configPath = join(agentDir(name), 'agent-config.json')
  try {
    const config = JSON.parse(readFileOr(configPath, '{}'))
    const raw = typeof config.accent === 'string' ? config.accent.trim() : ''
...
// Like listAgentNames() but INCLUDES technical/internal workers hidden via the
// sentinel. The reaper runner needs this: project agents spawned by NEXUS are
// internal (hidden from the dashboard roster + channel routing), yet they are
// exactly the ephemeral agents the reaper must find and tear down.
...
// Internal agents are TECHNICAL workers, not first-class channel agents: they
// stay out of the dashboard roster and inter-agent routing. The flag lives in
// agent-config.json AND is mirrored by the HIDDEN_AGENT_SENTINEL file (which is
// what listAgentNames() actually filters on).
export function isInternalAgent(name: string): boolean {
  try {
    if (existsSync(join(agentDir(name), HIDDEN_AGENT_SENTINEL))) return true
    const config = JSON.parse(readFileOr(join(agentDir(name), 'agent-config.json'), '{}'))
    return config.internal === true
  } catch { return false }
}

export function writeAgentInternal(name: string, internal: boolean): void {
  const configPath = join(agentDir(name), 'agent-config.json')
  let config: Record<string, unknown> = {}
  try { config = JSON.parse(readFileOr(configPath, '{}')) } catch {}
  config.internal = internal
  atomicWriteFileSync(configPath, JSON.stringify(config, null, 2))
  const sentinel = join(agentDir(name), HIDDEN_AGENT_SENTINEL)
  if (internal) {
