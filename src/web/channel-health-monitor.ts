...
// channel plugin connection drops. Glyph drifts across CC builds
// (✘ U+2718 in 2.1.168, ✗ U+2717 in older renders) -- match both.
...
function checkAgent(agentName: string, session: string): void {
  ...
  // Recovery check runs BEFORE the retry-cap/backoff gates: a plugin that
  // came back by other means (manual /mcp reconnect, channels.sh restart)
  // must clear the unhealthy state immediately. The old order skipped the
  // pane entirely while parked in backoff or the 30-min cooldown, so
  // getChannelHealth kept reporting healthy:false -- and the dashboard kept
  // showing the "Csatorna leszakadt" banner -- long after the plugin was
  // fine (2026-06-07 ~23:00 incident).
