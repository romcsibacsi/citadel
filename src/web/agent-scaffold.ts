...
import { PROJECT_ROOT, OWNER_NAME, MAIN_AGENT_ID, BOT_NAME, CHANNEL_PROVIDER, CHANNEL_CHAT_ID } from '../config.js'
import { channelStateDir } from '../channel-provider.js'
import { runAgent } from '../agent.js'
import { atomicWriteFileSync } from './atomic-write.js'
import { agentDir, AGENTS_BASE_DIR, readAgentSecurityProfile } from './agent-config.js'
...
// MAIN_AGENT_ID. The repo-side configs ship with `"agent": "nexus"`
// hardcoded (canonical default in src/config.ts) so a non-nexus
...
// Resolve the install-specific placeholders left in a seed roster's text
// files. The baked seed-agents/ files already have {{BOT_NAME}} (agent
// identity) and {{MAIN_AGENT_ID}} (self-scope) substituted at build time;
// {{OWNER_NAME}} and {{CHAT_ID}} stay as placeholders so each install gets
// its own operator name / channel id. {{BOT_NAME}}/{{MAIN_AGENT_ID}} are
// resolved here too as a defensive no-op in case a future seed leaves them.
function resolveSeedPlaceholders(content: string): string {
  return content
    .replaceAll('{{OWNER_NAME}}', OWNER_NAME || 'az operátorod')
...
// resolving placeholders in text files (.md / .json) and byte-copying the
// rest. Mirrors the conservative copy style of ensureDefaultScheduledTasks.
function copySeedDir(src: string, dest: string): void {
  mkdirSync(dest, { recursive: true })
  for (const entry of readdirSync(src)) {
    ...
    if (entry.endsWith('.md') || entry.endsWith('.json')) {
      atomicWriteFileSync(destPath, resolveSeedPlaceholders(readFileSync(srcPath, 'utf-8')))
    } else {
      copyFileSync(srcPath, destPath)
    }
  }
...
// Boot-time seeding of the fixed CITADEL sub-agent roster (FORGE, SPARK,
// SIGMA, RELAY, SCREENER, ORACLE). NEXUS is the main agent and lives at the
// repo root, so it is intentionally absent from seed-agents/.
//
// Idempotent and safe on every boot: an agent dir is only created when it
// does NOT already exist on disk, so an operator who has edited (or deleted)
// a seeded agent is never overwritten. After copying, the agent's runtime
// scaffolding is created and its security profile is materialized into
// .claude/settings.json, mirroring the /api/agents create flow, so a freshly
// seeded agent boots with the right permissions.
export function ensureSeedRoster(): string[] {
  const seedRoot = join(PROJECT_ROOT, 'seed-agents')
  if (!existsSync(seedRoot)) return []
  mkdirSync(AGENTS_BASE_DIR, { recursive: true })
  const seeded: string[] = []
  for (const name of readdirSync(seedRoot)) {
    const src = join(seedRoot, name)
    if (!statSync(src).isDirectory()) continue
    const dest = agentDir(name)
    if (existsSync(dest)) continue  // never overwrite an existing agent
    copySeedDir(src, dest)
    // Runtime scaffolding (memory, .mcp.json, settings.json hooks) then the
    // security profile -- same order as the create endpoint.
    scaffoldAgentDir(name)
    writeAgentSettingsFromProfile(name, loadProfileTemplate(readAgentSecurityProfile(name)))
    seeded.push(name)
  }
  return seeded
}
...
- Communication style
- Language rules (Hungarian with ${OWNER_NAME || 'az operátorod'}, English for code/technical)
- Tool usage guidelines relevant to the agent's role
- Any domain-specific instructions

${OWNER_NAME ? `The owner's name is ${OWNER_NAME}. Use this exact name everywhere the CLAUDE.md refers to the owner/user. Do not substitute or invent any other name.` : 'No personal owner name is configured. Refer to the owner/user neutrally as "az operátorod" (your operator); NEVER invent a personal name.'}
...
- How it addresses the user (${OWNER_NAME ? `whose name is ${OWNER_NAME} -- use this name, not any other` : 'neutrally as "az operátorod" -- no personal name, never invent one'})
...
2. Body with these sections:
   ...
   - ## Language rules - Hungarian with ${OWNER_NAME || 'az operátorod (semleges, nincs személynév)'} (the user), English for code/technical
   - ## What to avoid - common pitfalls

Keep the body under 200 lines. Be specific and actionable. ${OWNER_NAME ? `The owner's name is ${OWNER_NAME}; use only this name when referring to the user.` : 'No personal owner name is configured; refer to the owner neutrally as "az operátorod" and never invent a name.'}
Output ONLY the markdown content, no code fences.`
