...
export function startAgentProcess(name: string, opts: { fresh?: boolean } = {}): { ok: boolean; pid?: number; error?: string } {
  ...
  try {
    ...
    const stateEnvVar = agentProvider === 'discord' ? 'DISCORD_STATE_DIR' : 'TELEGRAM_STATE_DIR'
    const unsetTokens = 'unset TELEGRAM_BOT_TOKEN DISCORD_BOT_TOKEN'
...
// (resumeNexusSession / respawnNexusSessionFresh), which previously left the
