...
describe('ensureDefaultScheduledTasks JSON-rewrite (task-config.json)', () => {
  ...
  it('rewrites the agent field to MAIN_AGENT_ID on a hardcoded legacy-agent config', () => {
  ...
  it('preserves non-agent string fields containing the literal "legacy-agent"', () => {
    // Conservative scope: only the `agent` field is rewritten, even if
    // another field happens to contain the string "legacy-agent" (e.g. a
    // task `type` named "legacy-agent-heartbeat" in a hypothetical future
    // task). A blunt string replace would corrupt unrelated fields.
    const src = join(tmp, 'src.json')
    const dest = join(tmp, 'dest.json')
    writeFileSync(src, JSON.stringify({
      agent: 'legacy-agent',
      type: 'legacy-agent-heartbeat',
      description: 'pings the legacy-agent hub',
    }))
    rewriteAgentField(src, dest, 'host-agent')
    const parsed = JSON.parse(readFileSync(dest, 'utf-8'))
    expect(parsed.agent).toBe('host-agent')
    expect(parsed.type).toBe('legacy-agent-heartbeat')
    expect(parsed.description).toBe('pings the legacy-agent hub')
  })
})
